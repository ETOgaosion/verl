...
def cmd_prefetch(args: argparse.Namespace) -> int:
    ...
    git-sourced megatron-core / mbridge and the DeepSeek kernels (deep-ep /
    flash-mla / fast-hadamard-transform, no-build-isolation) are compiled in
    step 2, not here (apex / TE / flash-attn ship prebuilt from the wheelhouse,
    vllm / sglang / sglang-kernel from PyPI). The kernels need the system
    NVSHMEM / CCCL / arch build env baked by docker/Dockerfile.uv.cu130.
    ...
    # builds (megatron-core / mbridge and the DeepSeek kernels) build once,
